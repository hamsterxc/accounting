 ...
 *   body: array id => row-definition,
 *   bodyEdit: array id => row-definition,
 *   footer: row-definition,
 * }
 * row-definition = {
 *   id: row-id,
 *   cells: array name => row-cell-id,
 * }
 */
function populateDataTable(settings) {
    ...
    if (settings.selectorBody !== undefined) {
        ...
        rows.forEach(id => {
            ...
            result.body[id] = rowViewObject.info;
            result.bodyEdit[id] = rowEditObject.info;
...
function _createCell(tag, rowId, name, html, row, cells) {
    const id = rowId + '-' + name;
    const cell = $('<' + tag + '/>');
    cell.attr('id', id);
    cell.html(html);
